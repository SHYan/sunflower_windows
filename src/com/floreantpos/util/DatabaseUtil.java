...
public class DatabaseUtil {
	...
	public static boolean createDatabase(String connectionString, String hibernateDialect, String hibernateConnectionDriverClass, String user, String password,
		...
		try {
			...
			restaurant.setName(AppProperties.getAppName());
			...
			tg.setName("GovTax");
			tg.addTotaxes(tax);
			TaxGroupDAO.getInstance().saveOrUpdate(tg);
			
			
			Shift shift = new Shift();
			shift.setName(Messages.getString("MORNING_SHIFT"));
			java.util.Date shiftStartTime = ShiftUtil.buildShiftStartTime(10, 0, 0, 22, 0, 0);
			java.util.Date shiftEndTime = ShiftUtil.buildShiftEndTime(10, 0, 0, 22, 0, 0);
			...
			UserType administrator1 = new UserType();
			administrator1.setName("SUPER");
			administrator1.setPermissions(new HashSet<UserPermission>(Arrays.asList(UserPermission.permissions)));
			UserTypeDAO.getInstance().saveOrUpdate(administrator1);
			...
			server.setName("CASHIER");
			server.setPermissions(new HashSet<UserPermission>(Arrays.asList(UserPermission.RESTAURANT_MAINTENANCE,
					UserPermission.BACKUP_RESTORE,
					UserPermission.MENU_CATEGORIES_M,
					UserPermission.MENU_GROUPS_M,
					UserPermission.MENU_ITEMS_M,
					UserPermission.MENU_MODIFIER_GROUPS_M,
					UserPermission.MENU_MODIFIERS_M,
					UserPermission.INVENTORY_TRANSACTION,
					UserPermission.STOCK_ADJUST_REPORT,
					UserPermission.STOCK_BALANCE_REPORT,
					UserPermission.REPORT_DAILY_SALES_REPORT,
					UserPermission.REPORT_HOURLY_SALES_REPORT,
					UserPermission.REPORT_ORDER_DETAIL_REPORT,
					UserPermission.MENU_USAGE_REPORT,
					UserPermission.REPORT_Product_SALES_REPORT,
					UserPermission.REPORT_MODIFIER_SALES_REPORT,
					UserPermission.CREDIT_CARD_REPORT,
					UserPermission.REPORT_PAYOUT_REPORT,
					UserPermission.VIEW_ALL_OPEN_TICKETS,
					UserPermission.CREATE_TICKET,
					UserPermission.SPLIT_TICKET,
					UserPermission.MERGE_TICKET,
			...
			/*
			 * J1.1 3
			 * Modify some below default data
			 * */
			
			UserDAO dao = new UserDAO();
			/* Do not Edit 998 /999 */
			User administratorUser = new User();
			administratorUser.setUserId(998);
			administratorUser.setSsn("998");
			administratorUser.setPassword("0226981446");
			administratorUser.setFirstName("Sunflower");
			administratorUser.setLastName("User");
			administratorUser.setType(administrator1);
			...
			cashierUser.setPassword("09440663740");
			cashierUser.setFirstName("Tablet");
			cashierUser.setLastName("Default");
			cashierUser.setType(administrator1);
			cashierUser.setActive(true);
			dao.saveOrUpdate(cashierUser);
			
			administratorUser = new User();
			administratorUser.setUserId(1);
			administratorUser.setSsn("1");
			administratorUser.setPassword("3333");
			...
			cashierUser.setUserId(2);
			cashierUser.setSsn("2");
			cashierUser.setPassword("1111");
			cashierUser.setFirstName("Cashier");
			cashierUser.setLastName("User");
			cashierUser.setType(server);
			cashierUser.setActive(true);
			dao.saveOrUpdate(cashierUser);
			...
			CustomPaymentDAO cpd = new CustomPaymentDAO();
			cp.setName("KPAY");
			cp.setRefNumberFieldName("MOBILE NO");
			cp.setRequiredRefNumber(true);
			cpd.saveOrUpdate(cp);
			
			cp =  new CustomPayment();
			cp.setName("CBPAY");
			cp.setRefNumberFieldName("MOBILE NO");
			cp.setRequiredRefNumber(true);
			cpd.saveOrUpdate(cp);
			...
			cp =  new CustomPayment();
			cp.setName("WAVEMONEY");
			cp.setRefNumberFieldName("MOBILE NO");
			cp.setRequiredRefNumber(true);
			cpd.saveOrUpdate(cp);
			
			cp =  new CustomPayment();
			cp.setName("FOODPANDA");
			cp.setRefNumberFieldName("DRIVER INFO");
			cp.setRequiredRefNumber(true);
			cpd.saveOrUpdate(cp);
			...
			discount4.setName("5% Off");
			discount4.setType(1);
			discount4.setValue(5.0);
			discount4.setAutoApply(false);
			discount4.setMinimunBuy(1);
			discount4.setQualificationType(0);
			discount4.setApplyToAll(true);
			discount4.setNeverExpire(true);
			discount4.setEnabled(true);
			discountDao.saveOrUpdate(discount4);
			
			int terminalId = TerminalConfig.getTerminalId();

			if (terminalId == -1) {
				Random random = new Random();
				terminalId = random.nextInt(89999) + 10000;
			}
			...
			ActionHistoryDAO.getInstance().saveHistory(
					...
					"Initialize",
					"Create New Database and initialize default data!");
			DataImportAction.importMenuItems(DatabaseUtil.class.getResourceAsStream("/sample-menu-items.xml"));
