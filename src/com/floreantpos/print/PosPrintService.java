...
public class PosPrintService {
	...
	public static void printDrawerPullReport(DrawerPullReport drawerPullReport, Terminal terminal) {
		...
		try {
			...
			if (reportPrinter == null || reportPrinter.isEmpty()) {
				throw new PosException(Messages.getString("NO_RPC"));
			}
	...
	public static void printServerTipsReport(TipsCashoutReport report) {
		...
		try {
			...
			parameters.put("cashTips", NumberUtil.formatPrice(report.getCashTipsAmount())); //$NON-NLS-1$
			parameters.put("chargedTips", NumberUtil.formatPrice(report.getChargedTipsAmount())); //$NON-NLS-1$
			parameters.put("tipsDue", NumberUtil.formatPrice(report.getTipsDue())); //$NON-NLS-1$
