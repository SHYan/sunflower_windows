...
public class ShopTableButton extends PosButton {
	...
	public void update() {
		...
		/* J1.1 9*/
		ticketIdAsString = ticketIdAsString.replaceAll(",", ",<br>");
		...
		if (ticketCreateTime != null) {
			//ticketIdAsString = ticketIdAsString + "<br>" + DateUtil.getElapsedTime(ticketCreateTime, new Date());
			/* J1.1 10 */
			if(ticketCreateTime.toString()!=null)
				ticketIdAsString = ticketIdAsString + "<br>" + ticketCreateTime.toString().substring(0, 16);
		}
		...
		/* J1.1 7 */
		setText("<html><body><center><b>" + shopTable.getDescOrNum() + "<small>" + userName + ticketIdAsString + "</small></center></b></body></html>"); //$NON-NLS-1$
	...
	public boolean hasUserAccess() {
		...
		if (currentUser.hasPermission(UserPermission.PERFORM_ADMINISTRATIVE_TASK)) {
			return true;
		}
