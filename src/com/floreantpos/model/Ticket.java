...
public class Ticket extends BaseTicket {
	...
	public void addTableName(String tableName){
		List<String> names = getTableNames(true);
		if (names == null) {
			names = new ArrayList<String>();
			setTableNames(names);
		}else {
			if(names.contains(tableName)) return;
		}
		names.add(tableName);
		setTableNames(names);
			...
			for(int i=0; i<items.size(); i++) {
				...
				for(int j=0; j<sumItems.size(); j++) {
					...
					if(item.getName().equals(sItem.getName())) {
						...
						sumItems.get(j).setItemQuantity(sItem.getItemQuantity()+item.getItemQuantity());
	...
	public String getTitle() {
		...
		title += Messages.getString("Ticket.20") + ": " + NumberUtil.formatPrice(getTotalAmount()); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void calculatePrice() {
		...
		for (TicketItem ticketItem : ticketItems) {
			...
			/*J1.1 34 */
			itemQty += ticketItem.getItemQuantity();
		...
		*/
		...
		Double deliveryChargeAmount = NumberUtil.roundToDigitPrice(getDeliveryCharge());
		...
		setTotalAmount(NumberUtil.roundToDigitPrice(totalAmount));

		double dueAmount = totalAmount - getPaidAmount();
		setDueAmount(NumberUtil.roundToDigitPrice(dueAmount));
	}
	...
	private double calculateSubtotalAmount() {
		...
		return NumberUtil.roundToDigitPrice(subtotalAmount);
	}
	...
	private double calculateItemsDiscountAmount() {
		...
		return NumberUtil.roundToDigitPrice(ticketItemDiscounts);
	}
	...
	public double calculateToleranceAmount() {
		...
		return NumberUtil.roundToDigitPrice(discount);
	}
	...
	private double calculateTax(double discount, boolean isParcel) {
		...
		return NumberUtil.roundToDigitPrice(fixInvalidAmount(tax));
	}
	...
	public double calculateServiceCharge(double discount, Boolean isParcel) {
		...
		return NumberUtil.roundToDigitPrice(fixInvalidAmount(serviceCharge));
	}
