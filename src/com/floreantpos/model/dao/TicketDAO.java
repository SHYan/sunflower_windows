...
public class TicketDAO extends BaseTicketDAO {
	...
	public synchronized void saveOrUpdate(Ticket ticket) {
		...
		try {
			...
			//TODO
			/* BUG in this code, using this code will not save table number*/
			/* J1.1 54 ticket session refresh */
			/*
			if(ticket != null) {
				Ticket t = (Ticket) session.get(Ticket.class, ticket.getId());
				if(t!=null)
				session.refresh(t);
			}
			*/
	...
	private void adjustStockAmount(Ticket ticket, Session session) {
		...
		for (Integer ticketItemId : itemMap.keySet()) {
			...
			//J1.1 56
			if(!menuItem.isStockItem()) continue;
	...
	public List<Ticket> getTicketsWithSpecificFields(String... fields) {
		...
		boolean filterUser = !currentUser.isAdministrator();
		try {
			session = createNewSession();
	...
	/**
	 * J1.1 8
	 * to get the max id from ticket no other condition
	 * @return Integer
	 */
