...
public class TicketItemModifier extends BaseTicketItemModifier implements ITicketItem {
	...
	public void calculatePrice() {
		calculatePrice(0.0, false);
	}
	
	public void calculatePrice(double discount, Boolean isParcel) {
		if (isInfoOnly()) {
			return;
		}
		...
		calculateSubTotal();
		calculateTax();
		calculateServiceCharge(discount, isParcel);
		setTotalAmount(NumberUtil.roundToDigitPrice(calculateTotal()));
	}
	 ...
	 * J1.1 44
	 * */
	public void calculateServiceCharge(double discount, Boolean isParcel) {
		...
		if(serviceChargePercentage==0.0) return ;
		if(isParcel && TerminalConfig.isPcNoSc()) return ;
		if(!TerminalConfig.isSetSC()) return ;
	...
	private void calculateTax() {
		...
		setTaxAmount(NumberUtil.roundToDigitPrice(tax));
	}
	...
	private double calculateTotal() {
		...
		return getSubTotalAmount() + getTaxAmount() + getServiceCharge();
	}
	...
	private double calculateSubTotal() {
		...
		total = NumberUtil.roundToDigitPrice(getItemCount() * getUnitPrice());
		setSubTotalAmount(total);
		return total;
	}
