...
public class MemberSalesDetailReportView extends JPanel {
	...
	public MemberSalesDetailReportView() {
		...
		cbPrintType.setPreferredSize(new Dimension(125, 30));
		cbPrintType.setModel(new ListComboBoxModel(Arrays.asList("None", "Member Name", "Member Mobile")));
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Report Query", "Member Sales Detail Report");
				} catch (Exception e1) {
					POSMessageDialog.showError(MemberSalesDetailReportView.this, POSConstants.ERROR_MESSAGE, e1);
				}
			}
	...
	private void viewReport() throws Exception {
		...
		String reportTemplate = "member_sales_detail_report";
		
		if(cbPrintType.getSelectedItem().toString().equals("Member Name")) {
			dataSource = IBatisFactory.selectList("Report.getMember_Sales_Group_Name", map);
			if(dataSource!=null) {
				for(int i=0; i<dataSource.size(); i++) {
					//map.put("ticketId", dataSource.get(0).getId());
					map.put("cname", dataSource.get(i).getMember_name());
					dataSource.get(i).setItemList(new JRBeanCollectionDataSource(IBatisFactory.selectList("Report.getProduct_Sales_Member_Group", map)));
					//JOptionPane.showMessageDialog(null, i+" - "+map.get("cname"));

				}
			}
		}else if(cbPrintType.getSelectedItem().toString().equals("Member Mobile")) {
			dataSource = IBatisFactory.selectList("Report.getMember_Sales_Group_Mobile", map);
			if(dataSource!=null) {
				for(int i=0; i<dataSource.size(); i++) {
					//map.put("ticketId", dataSource.get(0).getId());
					map.put("mobileNo", dataSource.get(i).getMember_mobile());
					dataSource.get(i).setItemList(new JRBeanCollectionDataSource(IBatisFactory.selectList("Report.getProduct_Sales_Member_Group", map)));
				}
			}
		}else {
