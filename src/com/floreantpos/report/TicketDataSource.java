...
public class TicketDataSource extends AbstractReportDataSource {
	...
	public TicketDataSource() {
		super(new String[] { "itemName", "itemQty", "itemUnitPrice", "itemSubtotal" }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	}
	...
	public TicketDataSource(Ticket ticket, String viewOrderDetail) {
		super(new String[] { "itemName", "itemQty", "itemUnitPrice", "itemSubtotal" }); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$

		setTicket(ticket, viewOrderDetail);
	}

	private void setTicket(Ticket ticket, String viewOrderDetail) {
		...
		TicketItemRowCreator.calculateTicketRows(ticket, tableRows, null);
	...
	private void setTicket(Ticket ticket) {
		...
		LinkedHashMap<String, ITicketItem> tableRows = new LinkedHashMap<String, ITicketItem>();
		TicketItemRowCreator.calculateTicketRows(ticket, tableRows, "summary");
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		switch (columnIndex) {
			...
			case 1:
				...
				return format.format(Double.parseDouble(item.getItemQuantityDisplay()));
*/
			case 2:
				Double unitPrice = item.getUnitPriceDisplay();
				if (unitPrice == null) {
					return null;
				}

				return NumberUtil.formatPrice(unitPrice);
			...
			case 3:
				...
				return NumberUtil.formatPrice(total);
		}
