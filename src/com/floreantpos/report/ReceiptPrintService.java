...
public class ReceiptPrintService {
	...
	/* for order view only */
	public static JasperPrint createPrint(Ticket ticket, Map<String, Object> map) throws Exception {
		TicketDataSource dataSource = new TicketDataSource(ticket, "viewOrderDetail");
	...
	/* Call from OrderInfoDialog when click print button */
	/* Call from OrderInfoView printCopy(String) */
	...
	public static void printRefundTicket(Ticket ticket, RefundTransaction posTransaction) {
		try {
			...
			map.put("refundAmount", NumberUtil.formatPrice(posTransaction.getAmount())); //$NON-NLS-1$
			map.put("cashRefundText", Messages.getString("ReceiptPrintService.2")); //$NON-NLS-1$ //$NON-NLS-2$
			map.put("cashRefund", NumberUtil.formatPrice(posTransaction.getAmount())); //$NON-NLS-1$
	...
	public static void printTransaction(PosTransaction transaction) {
		try {
			...
			printProperties.setPrintCookingInstructions(false);
			printProperties.setPrintModifers(false);
	...
	public static HashMap populateTicketProperties(Ticket ticket, TicketPrintProperties printProperties, PosTransaction transaction) {
		...
		if(TerminalConfig.getUiDefaultFont().equals("Zawgyi-One")) {
			map.put("TEMPLATE_FILE", "reportStyle_zawgyi.jrtx");
        }else map.put("TEMPLATE_FILE", "reportStyle.jrtx");
        if(TerminalConfig.getQtyDecimal()>=0  && TerminalConfig.getQtyDecimal()<=2) 
        	map.put("QTY_TEMPLATE_FILE", "qtyStyle_"+TerminalConfig.getQtyDecimal()+".jrtx");
        if(TerminalConfig.getPriceDecimal()>=0  && TerminalConfig.getPriceDecimal()<=2) 
        	map.put("PRICE_TEMPLATE_FILE", "priceStyle_"+TerminalConfig.getPriceDecimal()+".jrtx");
		...
		map.put("customer", ticket.getProperty(Ticket.CUSTOMER_NAME));
		map.put("customerMobile", ticket.getProperty(Ticket.CUSTOMER_MOBILE));
		...
		if(TerminalConfig.getLogo()!=null && TerminalConfig.getLogo().length()>4)
			map.put("logoFile", "config"+File.separator+TerminalConfig.getLogo());
		else map.put("logoFile", "");
		...
		if (printProperties.isShowFooter()) {
			map.put("subtotalAmount", NumberUtil.formatPrice(ticket.getSubtotalAmount())); //$NON-NLS-1$
			double toleranceAmount = ticket.calculateToleranceAmount();
			if (toleranceAmount > 0.0) {
				map.put(ADJUST_AMOUNT, NumberUtil.formatPrice(toleranceAmount));
			}
			if (ticket.getDiscountAmount() > 0.0) {
				map.put(DISCOUNT_AMOUNT, NumberUtil.formatPrice(ticket.getDiscountAmount()));
			}

			if (ticket.getTaxAmount() > 0.0) {
				map.put(TAX_AMOUNT, NumberUtil.formatPrice(ticket.getTaxAmount()));
			}

			if (ticket.getServiceCharge() > 0.0) {
				map.put(SERVICE_CHARGE, NumberUtil.formatPrice(ticket.getServiceCharge()));
			}

			if (ticket.getDeliveryCharge() > 0.0) {
				map.put(DELIVERY_CHARGE, NumberUtil.formatPrice(ticket.getDeliveryCharge()));
			}

			if (ticket.getGratuity() != null) {
				tipAmount = ticket.getGratuity().getAmount();
				map.put(TIP_AMOUNT, NumberUtil.formatPrice(tipAmount));
			}
			map.put("totalAmount", NumberUtil.formatPrice(totalAmount)); //$NON-NLS-1$
			if (transaction != null) {
				map.put("tenderedAmount", NumberUtil.formatPrice(transaction.getTenderAmount())); //$NON-NLS-1$
			}
			map.put("paidAmount", NumberUtil.formatPrice(ticket.getPaidAmount())); //$NON-NLS-1$
			map.put("dueAmount", NumberUtil.formatPrice(ticket.getDueAmount())); //$NON-NLS-1$
			...
			if (transaction != null) {
				...
				if (changedAmount > 0) {
					map.put("changedAmount", NumberUtil.formatPrice(changedAmount)); //$NON-NLS-1$
				}
	...
	public static JasperPrint createPrint(KitchenTicket ticket) throws Exception {
		HashMap map = new HashMap();
		...
		if (ticket.getTableNumbers() != null && ticket.getTableNumbers().size() > 0) {
			map.put(TABLE_NO, POSConstants.RECEIPT_REPORT_TABLE_NO_LABEL + ticket.getTableNames());
		}
