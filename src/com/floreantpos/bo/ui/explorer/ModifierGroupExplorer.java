...
public class ModifierGroupExplorer extends TransparentPanel {
	...
	public void doEditSelectedModifierGroup() {
		try {
			...
			ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
					"Menu Manage", "Edit Menu Modifier Group "+category.getName());
		} catch (Throwable x) {
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "New Menu Modifier Group "+modifierGroup.getName());
				} catch (Throwable x) {
					BOMessageDialog.showError(com.floreantpos.POSConstants.ERROR_MESSAGE, x);
				}
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (ConfirmDeleteDialog.showMessage(ModifierGroupExplorer.this, com.floreantpos.POSConstants.CONFIRM_DELETE,
						...
						ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
								"Menu Manage", "Delete Menu Modifier Group "+category.getName());
					}
				} catch (Throwable x) {
					BOMessageDialog.showError(com.floreantpos.POSConstants.ERROR_MESSAGE, x);
				}
