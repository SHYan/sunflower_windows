...
public class MenuItemExplorer extends TransparentPanel {
	...
	private JPanel buildSearchForm() {
		...
		try {
			...
			panel.add(tfName);
			panel.add(lblGroup);
			cbGroup.setPreferredSize(new Dimension(150, 30));
			panel.add(cbGroup);
			cbOrderTypes.setPreferredSize(new Dimension(150, 30));
	...
	private TransparentPanel createButtonPanel() {
		...
		JButton btnChangeMenuGroup = new JButton(Messages.getString("CHANGE_GROUP"));
		JButton btnChangeOrderType = new JButton(Messages.getString("CHANGE_TYPE"));
		...
		duplicateButton.setPreferredSize(new Dimension(80, 40));
		updateStockAmount.setPreferredSize(new Dimension(140, 40));
		btnChangeMenuGroup.setPreferredSize(new Dimension(140, 40));
		btnChangeOrderType.setPreferredSize(new Dimension(140, 40));
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "Update Menu Item Stock Amount "+menuItem.getName() + " Amount : "+stockAmount);
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "New Menu Item "+foodItem.getName());
				} catch (Throwable x) {
					BOMessageDialog.showError(POSConstants.ERROR_MESSAGE, x);
				}
			}
		...
		duplicateButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					newMenuItem.setStockItem(existingItem.isStockItem());
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "Duplicate Menu Item "+foodItem.getName());
				} catch (Throwable x) {
					BOMessageDialog.showError(POSConstants.ERROR_MESSAGE, x);
				}
			}
		...
		btnChangeMenuGroup.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					searchItem();
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "Change Menu Item Group "+group.getName());
				} catch (Throwable x) {
					BOMessageDialog.showError(POSConstants.ERROR_MESSAGE, x);
				}
			}
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "Delete Menu Item "+item.getName());
				} catch (Throwable x) {
					BOMessageDialog.showError(POSConstants.ERROR_MESSAGE, x);
				}
			}
	...
	protected List<OrderType> getSelectedOrderTypes(List defaultValues) {
		...
		ComboItemSelectionDialog dialog = new ComboItemSelectionDialog(Messages.getString("OrderTypeSelectionDialog.0"), Messages.getString("KitchenDisplayView.6"), orderTypes, true);
