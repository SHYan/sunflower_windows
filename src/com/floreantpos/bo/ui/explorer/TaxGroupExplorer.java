...
public class TaxGroupExplorer extends TransparentPanel implements ActionListener, ListSelectionListener {
	...
	public TaxGroupExplorer() {
		...
		treeTable.setRowHeight(POSConstants.TABLE_ROW_HEIGHT);
	...
	private void createButtonPanel() {
		...
		JButton btn_newGroup = new JButton(Messages.getString("NEW_TAX_GROUP"));
		JButton btn_newTax = new JButton(Messages.getString("ADD_TAX"));
		JButton editButton = explorerButton.getEditButton();
		JButton deleteButton = explorerButton.getDeleteButton();

		btn_newGroup.setPreferredSize(new Dimension(120, 40));
		btn_newTax.setPreferredSize(new Dimension(70, 40));
		...
		btn_newGroup.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String groupName = JOptionPane.showInputDialog(POSUtil.getFocusedWindow(), Messages.getString("INPUT_GROUP_NAME"));
				if (groupName == null) {
					return;
				}
				if (groupName.equals("")) {
					BOMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("EMPTY_GROUP"));
					return;
				}

				if (groupName.length() > 30) {
					BOMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("LONG_GROUP_NAME"));
					return;
				}
				...
				ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
						"Menu Manage", "New Tax Group "+inventoryGroup.getName());
			}
		});
			...
			private void deleteInventoryTax() {
				try {
					...
					if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof TaxGroup) {
						...
						ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
								"Menu Manage", "Delete Tax Group "+attributeGroup.getName());
						...
						if (tableNode!=null && tableNode.getParent() != null) {
							noRootTreeTableModel.removeNodeFromParent(tableNode);
						}
						...
						taxDAO.delete(tax);
						ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
								"Menu Manage", "Delete Tax  "+tax.getName());
					}
	...
	private void editSelectedRow() {
		try {
			...
			if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof TaxGroup) {
				...
				String groupName = JOptionPane.showInputDialog(POSUtil.getFocusedWindow(), Messages.getString("GROUP_NAME"), attributeGroup.getName());
				if (groupName == null) {
					return;
				}
				...
				ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
						"Menu Manage", "Edit Tax Group "+attributeGroup.getName());
			}
			else if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof Tax) {
				...
				if (dialog.isCanceled())
					return;
				ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
						"Menu Manage", "Edit Tax  "+tax.getName());
			}
	...
	public Tax getSelectedAtrribute() {
		try {
			...
			if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof TaxGroup) {
				POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("SELECT_TAX"));
				return null;
			}
			else if (((DefaultMutableTreeTableNode) lastPathComponent).getUserObject() instanceof Tax) {
