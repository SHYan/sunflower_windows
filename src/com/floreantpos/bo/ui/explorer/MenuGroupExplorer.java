...
public class MenuGroupExplorer extends TransparentPanel {
	...
	public void doEditMenuGroup() {
		try {
			...
			ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
					"Menu Manage", "Edit Menu Group "+menuGroup.getName());
		} catch (Exception x) {
	...
	private void createButtonPanel() {
		...
		JButton btnChangeCategory = new JButton(Messages.getString("CHANGE_CATEGORY"));
		...
		btnChangeCategory.setPreferredSize(new Dimension(120, 40));
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "New Menu Group "+foodGroup.getName());
				} catch (Exception x) {
					BOMessageDialog.showError(POSConstants.ERROR_MESSAGE, x);
				}
			}
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "Delete Menu Group "+group.getName());
				} catch (Exception x) {
					BOMessageDialog.showError(POSConstants.ERROR_MESSAGE, x);
				}
			}
	...
	protected MenuCategory getSelectedMenuCategory(MenuCategory defaultValue) {
		...
		ComboItemSelectionDialog dialog = new ComboItemSelectionDialog(Messages.getString("MenuGroupForm.30"), Messages.getString("PosMessage.138"), menuCategorys, false);
