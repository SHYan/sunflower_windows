...
public class UserTypeExplorer extends TransparentPanel {
	...
	public void doEditSelectedUserType() {
		...
		try {
			...
			ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
					"BackOffice Setting", "Delete user type "+type.getName());
		} catch (Throwable x) {
		...
		addButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"BackOffice Setting", "Add user type "+type.getName());
				} catch (Exception x) {
					BOMessageDialog.showError(com.floreantpos.POSConstants.ERROR_MESSAGE, x);
				}
			}
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (ConfirmDeleteDialog
						...
						UserTypeDAO dao = new UserTypeDAO();
						category.clearPermissions();
						dao.delete(category);
						
						
						tableModel.deleteCategory(category, index);
						ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
								"BackOffice Setting", "Delete user type "+category.getName());
					}
				} catch (Exception x) {
					BOMessageDialog.showError(com.floreantpos.POSConstants.ERROR_MESSAGE, x);
				}
