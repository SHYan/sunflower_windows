...
public class ModifierExplorer extends TransparentPanel {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "New Menu Modifier "+modifier.getName());
				} catch (Throwable x) {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (ConfirmDeleteDialog
						...
						ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
								"Menu Manage", "Delete Menu Modifier "+category.getName());
					}
				} catch (Throwable x) {
					BOMessageDialog.showError(com.floreantpos.POSConstants.ERROR_MESSAGE, x);
				}
		...
		duplicateButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
							"Menu Manage", "Duplicate Menu Modifier "+menuModifier.getName());
				} catch (Throwable x) {
	...
	private void doEditSelectedMenuModifier() {
		try {
			...
			table.repaint();
			ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), 
					"Menu Manage", "Edit Menu Modifier "+modifier.getName());
		} catch (Throwable x) {
			BOMessageDialog.showError(com.floreantpos.POSConstants.ERROR_MESSAGE, x);
		}
	}
	...
	private JPanel buildSearchForm() {
		...
		panel.setLayout(new MigLayout("", "[][]30[][]30[]", "[]10[]")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		cbGroup.setPreferredSize(new Dimension(80, 30));
