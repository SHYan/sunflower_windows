...
public class InventoryTransactonAction extends AbstractAction {
	...
	public InventoryTransactonAction() {
		super(Messages.getString("INVENTORY_TRANSACTION"));
	}
	...
	public void actionPerformed(ActionEvent e) {
		String tab_name = Messages.getString("INVENTORY_TRANSACTION");
		BackOfficeWindow backOfficeWindow = com.floreantpos.util.POSUtil.getBackOfficeWindow();
		
		StockControlDialog trans = null;
		JTabbedPane tabbedPane = backOfficeWindow.getTabbedPane();
		int index = tabbedPane.indexOfTab(Messages.getString("INVENTORY_TRANSACTION"));
		if (index == -1) {
			trans = new StockControlDialog();
			tabbedPane.addTab(tab_name, trans);
		}
		else {
			trans = (StockControlDialog) tabbedPane.getComponentAt(index);
		}
