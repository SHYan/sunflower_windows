...
public class TicketViewerTable extends JTable {
	...
	public boolean decreaseFractionalUnit(double selectedQuantity) {
		int selectedRow = getSelectedRow();
	...
	private int getMaxReturnCount(int currentItemId) {
		Ticket t = getTicket();
		List<TicketItem>ticketList = t.getTicketItems();
		for(TicketItem ti : ticketList) {
			if(ti.getId()==currentItemId) {
	...
	public boolean increaseItemAmount() {
		int selectedRow = getSelectedRow();
		/*
		if (selectedRow < 0 || selectedRow >= model.getItemCount()) {
			return false;
		}
		 */
		...
		if (iTicketItem instanceof TicketItem) {
			...
			/* J1.1 19 prevent return item count increase >=0 
			if(ticketItem.getIsReturn() && ticketItem.getReturnItemId()!=null) {
				if(ticketItem.getItemCount() >=-1 )
					return false;
			}*/
			...
			if(ticketItem.getIsReturn()) {
				maxCountAble = getMaxReturnCount(ticketItem.getReturnItemId());
				if(Math.abs(itemCount) < maxCountAble)
					ticketItem.setItemCount(--itemCount);
			}else	ticketItem.setItemCount(++itemCount);
			/*
			double itemQuantity = ticketItem.getItemQuantity();
			if(itemQuantity > 0)
				ticketItem.setItemQuantity(itemQuantity+0.1);
			*/
	...
	public boolean decreaseItemAmount() {
		...
		if (iTicketItem instanceof TicketItem) {
			TicketItem ticketItem = (TicketItem) iTicketItem;
			/* J1.1 19 prevent return item decrease more than returned item count */
			/*
			if(ticketItem.getIsReturn() && ticketItem.getReturnItemId()!=null) {
				Ticket t = getTicket();
				for(TicketItem tm : t.getTicketItems()) {
					if(tm.getId() == ticketItem.getReturnItemId()) {
						if(tm.getItemCount()==0 && tm.getItemCount() <= Math.abs(ticketItem.getItemCount()))
							return false;
						
						//if(tm.getItemQuantity()!=0 && tm.getItemQuantity() <= Math.abs(ticketItem.getItemQuantity()))
							//return false;
					}
				}
			}*/
			...
			/* J1.1 34 */
			/*
			if (itemQuantity != 0 && itemQuantity<=0.1) {
				model.delete(selectedRow);
				fireTicketItemUpdated(getTicket(), ticketItem);
				return true;
			}*/
			if(ticketItem.getItemCount() > 0 )
				ticketItem.setItemCount(--itemCount);
			else ticketItem.setItemCount(++itemCount);//return item
			/*
			if(ticketItem.getItemQuantity() > 0 )
				ticketItem.setItemQuantity(itemCount-0.1);
				*/
			fireTicketItemUpdated(getTicket(), ticketItem);
			return true;
		}
	...
	public Object deleteSelectedItem() {
		...
		/* J1.1 19s prevent return item decrease more than returned item count */
		ITicketItem iTicketItem = (ITicketItem) model.get(selectedRow);
		if (iTicketItem instanceof TicketItem) {
			TicketItem ticketItem = (TicketItem) iTicketItem;
	...
	public void delete(int index, boolean forceDelete) {
		model.delete(index, forceDelete);
	}
