		...
		btnPay.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				if (selectedSection == null) {
					POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("SELECT_TICKET"));
					return;
				}
	...
	private class TicketSection extends JPanel implements MouseListener {
		...
		public void updateView() {
			...
			if (splitTicket == null) {
				...
				lblTotalAmount.setText(currencySymbol + NumberUtil.formatPrice(splitTicket.getTotalAmount()));
				...
				if (splitTicket.isRefunded()) {
					...
					dueStatusText = currencySymbol + NumberUtil.formatPrice(splitTicket.getDueAmount());
				}
				...
				if (splitTicket == null) {
					...
					if (splitTicket.getOrderType().isShouldPrintToKitchen()) {
						if (splitTicket.needsKitchenPrint()) {
							...
							POSMessageDialog.showMessage(Messages.getString("TICKET_SENT"));
