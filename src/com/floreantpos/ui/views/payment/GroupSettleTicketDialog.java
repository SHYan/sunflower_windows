...
public class GroupSettleTicketDialog extends POSDialog implements CardInputListener {
	...
	public void updateView() {
		...
		tfSubtotal.setText(NumberUtil.formatPrice(subtotalAmount));
		tfDiscount.setText(NumberUtil.formatPrice(discountAmount));

		tfDeliveryCharge.setText(NumberUtil.formatPrice(deliveryCharge));

		if (Application.getInstance().isPriceIncludesTax()) {
			tfTax.setText(Messages.getString("TicketView.35")); //$NON-NLS-1$
		}
		else {
			tfTax.setText(NumberUtil.formatPrice(taxAmount));
		}
		if (gratuityAmount > 0) {
			tfGratuity.setText(NumberUtil.formatPrice(gratuityAmount));
		}
		else {
			tfGratuity.setText("0.00"); //$NON-NLS-1$
		}
		tfTotal.setText(NumberUtil.formatPrice(totalAmount));
	...
	public void doGroupSettle(PaymentType paymentType) {
		try {
			...
			totalDueAmount = NumberUtil.roundToDigitPrice(totalDueAmount);
