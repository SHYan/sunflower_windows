...
public class PaymentView extends JPanel {
	...
	private void initComponents() {
		...
		calcButtonPanel.add(btnAmount1000, "grow");
		...
		calcButtonPanel.add(btnAmount500, "grow"); 
		...
		btnExactAmount.setText((TerminalConfig.isSetTax()) ? Messages.getString("NO_TAX") : Messages.getString("SET_TAX"));
		...
		btnNoSale.setText((TerminalConfig.isPrintReceipt()) ? Messages.getString("NO_RECEIPT_PRINT") : Messages.getString("RECEIPT_PRINT"));
		btnNoSale.setActionCommand("printReceipt");
		...
		btnNextAmount.setText((TerminalConfig.isSetSC()) ? Messages.getString("NO_SC") : Messages.getString("SET_SC"));
		
		btnNextAmount.setActionCommand("SC"); //$NON-NLS-1$
		...
		/* Only for HC Happy Chicken 2 */
		btnGratuity = new PosButton("HC RETURN");
		/*
		btnGratuity.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(ticket.getCustomer()==null) {
					POSMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("OrderTypeForm.6"));
					return;
				}
				if(ticket.getCustomer().getZipCode()==null) {
					POSMessageDialog.showError(POSUtil.getFocusedWindow(), "Need Customer Zip Code (Same with Warehouse ID)");
					return;
				}
				int zip = 0;
				try {
					zip = Integer.parseInt(ticket.getCustomer().getZipCode());
				}catch(Exception e1) {
					POSMessageDialog.showError(POSUtil.getFocusedWindow(), "Need Customer Zip Code (Same with Warehouse ID)");
					return;
				}
				...
				List<TicketItem> ti = ticket.getTicketItemsSummary();
				if(ti==null) return;
				Date fromDate = DateUtil.startOfDay(new Date(), 0, 0 );
				Date toDate = DateUtil.endOfDay(new Date(), 23, 59);
				map.put("startDate", fromDate);
				map.put("endDate", toDate);
				map.put("warehouseId", zip);
				String transferQuery = "inventory_transaction.getProduct_Stock_Return_Qty";
				for(TicketItem t: ti ) {
					map.put("item_id", t.getMenuItem().getId());
					map.put("tran_type", "Return");
					map.put("warehouse_id", zip);
					Integer transferAll = (Integer) IBatisFactory.selectOneSQL(transferQuery, map);
					map.put("old_qty", transferAll);
					map.put("qty", t.getItemCount()==0 ? -1*(transferAll+t.getItemQuantity()) : -1*(transferAll+t.getItemCount()));
					
					
					Date d = new Date();
					Timestamp ts = new Timestamp(d.getTime());
					map.put("transaction_date", ts);
					if((transferAll+t.getItemQuantity())==0 || (transferAll+t.getItemCount())==0) continue;
				...
				POSMessageDialog.showMessage("Return Finish"); //$NON-NLS-1$
				btnGratuity.setVisible(false);
			}
		});
		*/
		...
		btnMultiCurrencyCash.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				try {
					...
					double x = NumberUtil.parsePrice(txtTenderedAmount.getText()).doubleValue();
		...
		btnDebitCard = new PosButton("FOODPANDA");
		actionButtonPanel.add(btnDebitCard, "grow, w " + width + "!");
		btnDebitCard.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					ticketProcessor.doSettle(PaymentType.FOOD_DELIVERY, getTenderedAmount());
				} catch (StaleStateException x) {
					POSMessageDialog.showMessageDialogWithReloadButton(POSUtil.getFocusedWindow(), refreshableView);
				} catch (Exception exception) {
	...
	protected boolean adjustCashDrawerBalance(List<Currency> currencyList) {
		...
		txtTenderedAmount.setText(NumberUtil.formatPrice(dialog.getTenderedAmount()));
	...
	Action nextButtonAction = new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (command.equals("SC")) { //$NON-NLS-1$
					//double dueAmount = getDueAmount();
					//txtTenderedAmount.setText(NumberUtil.formatPrice(dueAmount));
					TerminalConfig.setSC((TerminalConfig.isSetSC() ?  false : true));
					btnNextAmount.setText((TerminalConfig.isSetSC()) ? Messages.getString("NO_SC") : Messages.getString("SET_SC"));
					doModifyTax();
				}
				//J1.1 32
				...
				//J1.133
				else if (command.equals("printReceipt")) {
					
					//btnNoSale.setText((TerminalConfig.isPrintReceipt()) ? Messages.getString("PRINT_REC") : Messages.getString("NOT_PRINT_REC"));
					...
					double x = NumberUtil.parsePrice(txtTenderedAmount.getText()).doubleValue();//FIXME: what if exception occurs?
	...
	public void updateView() {
		...
		txtDueAmount.setText(NumberUtil.formatPrice(dueAmount));
		txtTenderedAmount.setText(NumberUtil.formatPrice(dueAmount));
	}

	public double getTenderedAmount() throws ParseException {
		double doubleValue = NumberUtil.parsePrice(txtTenderedAmount.getText()).doubleValue();
		return doubleValue;
	}
	...
	private void doPayByCash() {
		try {
			...
			if (ticket == null || ticket.getTicketItems() == null || ticket.getTicketItems().size() == 0) {
				POSMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("PosMessage.175"));
				return;
			}
			/* Only for HC Happy Chicken 1 
			if(ticket.getCustomer()==null) {
				POSMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("OrderTypeForm.6"));
				return;
			}*/

			double terderAmount = NumberUtil.parsePrice(txtTenderedAmount.getText()).doubleValue();
