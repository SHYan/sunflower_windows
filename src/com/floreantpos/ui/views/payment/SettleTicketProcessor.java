...
public class SettleTicketProcessor implements CardInputListener {
	...
	public String getPaymentSubType(String name) {
		switch (name) {
			case "KPAY":
			case "CBPAY":
			case "AYAPAY":
	...
	public void doSettle(PaymentType paymentType, double tenderAmount, CustomPayment customPayment) throws Exception {
		...
		switch (paymentType) {
			...
			case CUSTOM_PAYMENT:
				...
				transaction.setPaymentType(getPaymentSubType(customPaymentDialog.getPaymentName()));
				transaction.setTicket(ticket);
				transaction.setCaptured(true);
				setTransactionAmounts(transaction);
			...
			case MOBILE_BANKING:
				...
				CustomPayment payment = CustomPaymentDAO.getInstance().getByName("KPAY");
				String field = payment.getRefNumberFieldName();
				String ref="";
				...
				if (payment.isRequiredRefNumber()) {
					...
					if (dialog.isCanceled())
						return;
					ref = dialog.getPaymentRef();
				}

				transaction = paymentType.createTransaction();
				transaction.setCustomPaymentFieldName(field);
				transaction.setCustomPaymentName(payment.getName());
				transaction.setCustomPaymentRef(ref);
				transaction.setPaymentType(getPaymentSubType(payment.getName()));
				transaction.setTicket(ticket);
				transaction.setCaptured(true);
				setTransactionAmounts(transaction);
			...
			case FOOD_DELIVERY:
				...
				CustomPayment payment1 = CustomPaymentDAO.getInstance().getByName("FOODPANDA");
				String field1 = payment1.getRefNumberFieldName();
				String ref1="";

				if (payment1.isRequiredRefNumber()) {

					PaymentReferenceEntryDialog dialog = new PaymentReferenceEntryDialog(payment1);
					dialog.pack();
					dialog.open();

					if (dialog.isCanceled())
						return;
					ref1 = dialog.getPaymentRef();
				}

				transaction = paymentType.createTransaction();
				transaction.setCustomPaymentFieldName(field1);
				transaction.setCustomPaymentName(payment1.getName());
				transaction.setCustomPaymentRef(ref1);
				transaction.setPaymentType(getPaymentSubType(payment1.getName()));
				transaction.setTicket(ticket);
				transaction.setCaptured(true);
				setTransactionAmounts(transaction);
