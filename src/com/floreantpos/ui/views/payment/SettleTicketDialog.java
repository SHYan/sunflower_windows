...
public class SettleTicketDialog extends POSDialog implements PaymentListener, TicketViewerTableChangeListener, RefreshableView {
	...
	public void updateView() {
		...
		if (Application.getInstance().isPriceIncludesTax()) {
			...
			tfServiceCharge.setText(NumberUtil.formatPrice(ticket.getServiceCharge()));
		}
		else {
			tfServiceCharge.setText("0.00"); //$NON-NLS-1$
		}
		/* J1.1 34
	...
	private JPanel createTotalViewerPanel() {
		...
		lblDeliveryCharge.setText(Messages.getString("SALES_DELIVERY_CHARGE")+":" + " " + CurrencyUtil.getCurrencySymbol()); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private void payUsingPreAuthorizedBartab(PosTransaction bartabTransaction) throws Exception {
		...
		double tipsAmount = NumberSelectionDialog2.takeDoubleInput(Messages.getString("ENTER_TIP"), 0.0);
		if (tipsAmount > 0) {
