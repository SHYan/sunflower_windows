		...
		ticketViewerTable.addMouseListener(new MouseAdapter() {
		    public void mousePressed(MouseEvent mouseEvent) {
		    	if (mouseEvent.getClickCount() == 2) {
		    		...
		    		int pos = ticketViewerTable.getSelectedRow();
		    		TicketItem ticketItem = ticketItems.get(pos);
		    		
		    		if(ticketItem.isFractionalUnit()) {
		    			double fQty = BasicWeightInputDialog.takeDoubleInput(Messages.getString("ENTER_NEW_FARCTION"), Messages.getString("QUANTITY_INPUT"), ticketItem.getItemQuantity());
		    			if (fQty > 0) {
			    			ticketItem.setItemQuantity(fQty);
			    		}else POSMessageDialog.showError(Messages.getString("QTY_ERROR"));
		    		} else if(TerminalConfig.isCustomQty()){
		    			int fQty = BasicWeightInputDialog.takeIntInput(Messages.getString("ENTER_NEW_QTY"), Messages.getString("QUANTITY_INPUT"), ticketItem.getItemCount());
		    			if (fQty > 0) {
			    			ticketItem.setItemCount(fQty);
			    		}else POSMessageDialog.showError(Messages.getString("QTY_ERROR"));
		    		}
					
		    		if(TerminalConfig.isOpenPrice() && permissions.contains(UserPermission.OPEN_PRICE)) {
		    			double unitPrice = BasicWeightInputDialog.takeDoubleInput(Messages.getString("ENTER_NEW_PRICE"), Messages.getString("UNIT_PRICE"), ticketItem.getUnitPrice());
		    			if (unitPrice >= 0) {
			    			ticketItem.setUnitPrice(unitPrice);
			    		}else POSMessageDialog.showError(Messages.getString("UNIT_PRICE_ERROR"));
		    		}
	...
	private void createItemSearchPanel() {
		...
		btnSearch.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				dialog.setTitle(Messages.getString("SEARCH_ITEM_BUTTON_TEXT"));
				dialog.pack();
				dialog.open();
...
//		}
	...
	private void createTicketItemControlPanel() {
		...
		btnIncreaseAmount.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				if (selectedTicketItem == null || selectedTicketItem.isPrintedToKitchen()) {
					return;
				}
				if(ticketViewerTable.getSelected().getClass().equals(TicketItemModifier.class)) {
					OrderController.openModifierDialog(ticketViewerTable.getSelected());
					updateView();
					return;
				}
	...
	private void doDeleteSelection() {// GEN-FIRST:event_doDeleteSelection
		 ...
		 * J1.1 16s
		 * */
		if(ticketViewerTable.getSelected()==null) return;
		
		ITicketItem iti = ticketViewerTable.getSelected();
		int oldSelected = ticketViewerTable.getSelectedRow();
		
		ticketViewerTable.deleteSelectedItem();
		
		int newSelected = 0;
		if(oldSelected == 0) newSelected=0;
		else newSelected = oldSelected-1;
		int totalRow = ticketViewerTable.getRowCount();
		
		if(iti.getClass().equals(TicketItem.class)) {
			if(((TicketItem)iti).getIsReturn()) {
				newSelected = totalRow-1;
			}
		}
		
		if( totalRow>0 )
			ticketViewerTable.setRowSelectionInterval(newSelected, newSelected);
		updateView();
	...
	private void doEditSelection() {// GEN-FIRST:event_doDeleteSelection
		...
		if (object instanceof TicketItem && ((TicketItem) object).isTreatAsSeat()) {
			...
			seatDialog.setTitle(Messages.getString("SELECT_SEAT"));
			seatDialog.pack();
			seatDialog.open();
			...
			if (seatNumber == -1) {
				...
				dialog.setTitle(Messages.getString("ENTER_SEAT"));
	...
	private void doIncreaseAmount() {// GEN-FIRST:event_doIncreaseAmount
		...
		if(ticketViewerTable.getSelected().getClass().equals(TicketItem.class)) {
			if (!checkStock(((TicketItem)ticketViewerTable.getSelected()).getItemCount()+1)) {
				POSMessageDialog.showError(Messages.getString("ITEM_NOT_STOCK"));
				return;
			}
	...
	/* J1.1 43 */
	private void doDecreaseFractionalUnit() {
		...
		if(ticketViewerTable.getSelected().getClass().equals(TicketItem.class)) {
			if (ticketViewerTable.decreaseFractionalUnit(selectedQuantity)) {
				updateView();
			}
		}
	}
	...
	public void updateView() {
		...
		btnTotal.setText(POSConstants.TOTAL.toUpperCase() + " " + CurrencyUtil.getCurrencySymbol() + NumberUtil.formatPrice(ticket.getTotalAmount()));
		/*if (ticket.getTotalAmount() > 0) {
			//btnTotal.setText("<html><h2>Total " + Application.getCurrencySymbol() + NumberUtil.formatPrice(ticket.getTotalAmount()) + "</h2></html>");
			btnTotal.setText("Total " +Application.getCurrencySymbol() + NumberUtil.formatPrice(ticket.getTotalAmount()));
		}
		else {
	...
	private double getNewItemQuantity() {
		ITicketItem selectedTicketItem = ticketViewerTable.getSelected();
		double oldQty = ((TicketItem)selectedTicketItem).getItemQuantity();
		...
		if (TerminalConfig.getScaleActivationValue().equals("cas10")) {
			...
			selectedQuantity = BasicWeightInputDialog.takeDoubleInput(Messages.getString("ENTER_WEIGHT"), Math.abs(oldQty));
			
		}
		

		if (selectedQuantity == 0) {
			POSMessageDialog.showError(Messages.getString("UNIT_ZERO"));
			return 0;
		}
		if(oldQty < 0) {
			if(Math.abs(oldQty) < Math.abs(selectedQuantity)) return oldQty;
			return selectedQuantity * (-1);
		}
		else return selectedQuantity;
	}
	...
	private boolean checkStock(double selectedItemQuantity) {
		if(!TerminalConfig.isCheck_stock()) return true;
