...
public class OrderController implements OrderListener, CategorySelectionListener, GroupSelectionListener, ItemSelectionListener {
	...
	public void itemSelected(MenuItem menuItem) {
		...
		double itemQuantity = 0, itemUnitPrice = menuItem.getPrice();
		if (menuItem.isFractionalUnit()) {
			if (TerminalConfig.getScaleActivationValue().equals("cas10")) {
				...
				itemQuantity = BasicWeightInputDialog.takeDoubleInput(Messages.getString("ENTER_WEIGHT"), 1);
			}
			if (itemQuantity <= -1) {
				return;
			}
			...
			if(TerminalConfig.isFractionAutoOP()) {
    			itemUnitPrice = BasicWeightInputDialog.takeDoubleInput(Messages.getString("ENTER_NEW_PRICE"), Messages.getString("UNIT_PRICE"), menuItem.getPrice());
    			if (itemUnitPrice < 0) {
    				POSMessageDialog.showError(Messages.getString("UNIT_PRICE_ERROR"));
	    		}
    		}
		}
		...
		/*
		if(itemUnitPrice >= 0)
			ticketItem.setUnitPrice(itemUnitPrice);
		*/
	...
	public synchronized static void saveOrder(Ticket ticket) {
		...
		/* J1.1 8s 
		 * If this is new ticket, generate new ticket id according to date format with 3 digits
		 * */
		if(newTicket) {
			
			ticket.setId(ticket.generateNewTicketId(ticketDAO));
		...
		if (newTicket) {
			...
			actionHistoryDAO.saveHistory(user, ActionHistory.NEW_CHECK, "Seq : "+ ticket.getId()+"; ItemCount : "+ticket.getItemCount()+"; Amount : "+ticket.getTotalAmount()); //$NON-NLS-1$
		} 
		/*else {
