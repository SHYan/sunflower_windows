...
public class OrderView extends ViewPanel implements PaymentListener, TicketEditListener, RefreshableView {
	...
	private void addActionButtonPanel() {
		...
		btnCancel.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				...
				int result = POSMessageDialog.showYesNoQuestionDialog(null, Messages.getString("ITEM_NOT_SEND_KITCHEN"), Messages.getString("Application.2"));
		...
		btnSend.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					POSMessageDialog.showMessage(Messages.getString("ITEM_SENT"));
				} catch (StaleStateException x) {
					POSMessageDialog.showMessageDialogWithReloadButton(POSUtil.getFocusedWindow(), getInstance());
				} catch (PosException x) {
	...
	private JPanel createTicketSummeryPanel() {
		...
		*/
		tfService = new javax.swing.JTextField(10);
		tfService.setEditable(false);
		tfService.setHorizontalAlignment(SwingConstants.TRAILING);
		...
		ticketAmountPanel.add(tfService, "growx,aligny center"); //$NON-NLS-1$
	...
	public void updateTicketSummeryView() {
		...
		if (ticket == null) {
			...
			//J1.1 34
			//tfGratuity.setText(""); //$NON-NLS-1$
			tfService.setText(""); //$NON-NLS-1$
			return;
		}
		tfSubtotal.setText(NumberUtil.formatPrice(ticket.getSubtotalAmount()));
		tfDiscount.setText(NumberUtil.formatPrice(ticket.getDiscountAmount()));
		tfDeliveryCharge.setText(NumberUtil.formatPrice(ticket.getDeliveryCharge()));

		if (Application.getInstance().isPriceIncludesTax()) {
			tfTax.setText(Messages.getString("TicketView.35")); //$NON-NLS-1$
		}
		else {
			tfTax.setText(NumberUtil.formatPrice(ticket.getTaxAmount()));
		}
		
		
		/* J1.1 34
		if (ticket.getGratuity() != null) {
			tfGratuity.setText(NumberUtil.formatPrice(ticket.getGratuity().getAmount()));
		}
		else {
			tfGratuity.setText("0.00"); //$NON-NLS-1$
		}*/
		if (ticket.getServiceCharge() != null) {
			tfService.setText(NumberUtil.formatPrice(ticket.getServiceCharge()));
		}
		else {
			tfService.setText("0.00"); //$NON-NLS-1$
		}
		
		tfTotal.setText(NumberUtil.formatPrice(ticket.getTotalAmount()));
	...
	public void updateTableNumber() {
		try {
			...
			String oldTable = "";
			try {
				oldTable = currentTicket.getTableNames().get(0);
			}catch(Exception e ) {
				oldTable = "XXX";
			}
			*/
			...
			actionUpdate();
			//ReceiptPrintService.printTransferTable(currentTicket, oldTable);
		...
		if (currentTicket != null) {
			...
			if (!type.isShowTableSelection()) {
				...
				if (tableNumbers != null) {
					...
					String tables = (currentTicket.getTableNames()==null) ? "" :  getTableNames(currentTicket.getTableNames(true));
					/* J1.1 7e */
	...
	private String getTableNames(List<String> array) {
		String result = "";
	...
	private void setVisibleButtonForOrderView() {
		...
		if(Application.getCurrentUser().hasPermission(UserPermission.ADD_MISC_ITEM))
			btnMisc.setVisible(true);
		else btnMisc.setVisible(false);
		btnSeatNo.setVisible(true);
	...
	public void updateView() {
		...
		if(currentTicket!=null && currentTicket.getCustomer()!=null) {
			btnCustomer.setText("<html><body><center>CUSTOMER<br>\"" + currentTicket.getCustomer().getName() + "\"</center></body></html>");
	...
	public void paymentCanceled() {
		...
		if (currentTicket.getId() != null && currentTicket.getDueAmount() > 0.0) {
			POSMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("PAYMENT_NOT_COMPLETE"));
			return;
		}
