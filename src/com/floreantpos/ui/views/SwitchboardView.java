...
public class SwitchboardView extends ViewPanel implements ActionListener, ITicketList, TicketListUpdateListener {
	...
	private List<Ticket> mergeTicketList = new ArrayList<Ticket>();
...
*/
		...
		btnNewBarTab.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (barTabOrders.size() > 1) {
					...
					dialog.setTitle(Messages.getString("OrderTypeSelectionDialog.0"));
				...
				if (!orderType.isBarTab()) {
					POSMessageDialog.showMessage(Messages.getString("ORDER_TYPE_NOTBAR"));
					return;
				}
	...
	private JPanel createActivityPanel() {
		...
		if (Application.getInstance().getTerminal().isHasCashDrawer()) {
			...
			//secondRowButtonPanel.getContentPane().add(btnSettleTicket);
...
//		}
	...
	private void doMergeTicket() {
		try {
			...
			if(ticket.getClosingDate() != null) {
				POSMessageDialog.showMessage(Messages.getString("MERGE_CLOSE_TICKET"));
				return;
			}
			if(mergeTicketList.size()==0){
				int alarm = POSMessageDialog.showYesNoQuestionDialog(Application.getPosWindow(),
						Messages.getString("SELECT_ANOTHER_MERGE_TICKET"), Messages.getString("Confirmation") );
				if (alarm == JOptionPane.NO_OPTION) {
					mergeTicketList.clear();
					return;
				}
			}	
			mergeTicketList.add(ticket);
			
			//merge ticket
			if(mergeTicketList.size()==2){
				Integer t1=mergeTicketList.get(0).getId(), t2=mergeTicketList.get(1).getId();
				if(t1 == t2){
					POSMessageDialog.showMessage(Messages.getString("SAME_CHOOSE_ANOTHER_TO_MERGE"));
					mergeTicketList.remove(1);
				}else{
					int option = POSMessageDialog.showYesNoQuestionDialog(Application.getPosWindow(),
							Messages.getString("CONFIRM_MERGE_TICKET")+t1+" + "+t2, Messages.getString("Confirmation") ); //$NON-NLS-1$ //$NON-NLS-2$
					if (option == JOptionPane.NO_OPTION) {
						POSMessageDialog.showMessage(Messages.getString("CANCEL_MERGE_TICKET"));
						mergeTicketList.clear();
						return;
					}
					
					Ticket firstticket = TicketDAO.getInstance().loadFullTicket(t1);
					...
					firstticket.setTaxRate(ticket.getTaxRate());
					...
					for (TicketItem item : ticketitems) {
					    item.setTicket(mergeTicketList.get(0));
					}
					try{
						...
						POSMessageDialog.showMessage("You cannot merge same ticket. Select Ticket is "+t1+".");	
					}
					
					mergeTicketList.clear();
					updateView();
					ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), "MERGE CHECK",
							 "Seq:" + t1+" + "+t2); //$NON-NLS-1$
				}
			}
	...
	private void doSplitTicket() {
		try {
			...
			Ticket ticket = TicketDAO.getInstance().loadFullTicket(selectedTicket.getId());
			if(ticket.getClosingDate() != null) {
				POSMessageDialog.showMessage(Messages.getString("SPLIT_CLOSE_TICKET"));
				return;
			}
	...
	public Ticket getSelectedTicket() {
		return ticketList.getSelectedTicket();
		/*
		List<Ticket> selectedTickets = ticketList.getSelectedTickets();
