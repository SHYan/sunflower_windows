...
public class QuickCustomerForm extends BeanEditor<Customer> {
	...
	private void createCustomerForm() {
		...
		inputPanel.setBorder(BorderFactory.createTitledBorder(Messages.getString("ENTER_CUSTOMER_INFO")));
		...
		tfCellPhone.setPreferredSize(new Dimension(195, 30));
	...
	public boolean delete() {
		try {
			...
			int option = POSMessageDialog.showYesNoQuestionDialog(POSUtil.getBackOfficeWindow(), Messages.getString("ShopTableForm.14"), Messages.getString("ShopTableForm.15")); //$NON-NLS-1$ //$NON-NLS-2$
