...
public class CurrencyForm extends BeanEditor {
	...
	private void initComponents() {
		...
		JLabel lblCode = new JLabel(Messages.getString("CURRENCY_CODE")+" : ");
		...
		JLabel lblExchangeRate = new JLabel(Messages.getString("CURRENCY_RATE")+" : ");
		tfExchangeRate = new DoubleTextField();

		lblTolerance = new JLabel(Messages.getString("CURRENCY_TOLERANCE")+" : ");
		tfTolerance = new DoubleTextField();

		JLabel lblSymbol = new JLabel(Messages.getString("CURRENCY_SYMBOL")+" : ");
	...
	protected boolean updateModel() {
		...
		if (POSUtil.isBlankOrNull(code)) {
			POSMessageDialog.showError(POSUtil.getFocusedWindow(), Messages.getString("CODE_REQ"));
			return false;
		}
		...
		if (chkMain.isSelected()) {
			if (exchangeRate != 1.0) {
				POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("CURRENCY_RATE_ERR"));
				return false;
			}
		}
		double tolerance = tfTolerance.getDoubleOrZero();
		if (tolerance > 0 && !chkMain.isSelected()) {
			POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("CHECK_MAIN"));
	...
	public String getDisplayText() {
		...
		if (currency.getId() == null) {
			return Messages.getString("NEW_CURRENCY");
		}
		return Messages.getString("CurrencyDialog.0");
	}
}
