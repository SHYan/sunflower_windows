...
public class PayoutDialog extends OkCancelOptionDialog {
	...
	public void doOk() {
		...
		payOutTransaction.setCardExtData(reason.getReason());
		payOutTransaction.setCardARQC(recepient.getName());
		...
		try {
			...
			actionMessage += "To:" + recepient + ", For:"+reason+", Amount:" + NumberUtil.formatPrice(payoutAmount); //$NON-NLS-1$ //$NON-NLS-2$
			ActionHistoryDAO.getInstance().saveHistory(Application.getCurrentUser(), ActionHistory.PAY_OUT, actionMessage);
			POSMessageDialog.showMessage(Messages.getString("DatabaseConfigurationDialog.27")); //$NON-NLS-1$
			payOutView.resetPayoutAmount();
			//dispose();
		} catch (Exception e) {
