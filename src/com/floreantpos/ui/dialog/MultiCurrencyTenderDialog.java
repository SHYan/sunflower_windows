...
public class MultiCurrencyTenderDialog extends OkCancelOptionDialog {
	...
	private void init() {
		...
		setTitle(Messages.getString("ENTER_TENDER_AMOUNT"));
		setTitlePaneText(Messages.getString("PaymentView.52") + dueAmount);
		setResizable(true);
	...
	public void doOk() {
		...
		if (!isValidAmount()) {
			POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("PaymentView.32"));//$NON-NLS-1$
			return;
		}
		...
		for (CurrencyRow rowItem : currencyRows) {
			...
			item.setBalance(NumberUtil.roundToDigitPrice(item.getBalance() + tenderAmount - cashBackAmount));
		}
	...
	private boolean isValidAmount() {
		double remainingBalance = NumberUtil.roundToDigitPrice(dueAmount - (totalTenderedAmount - totalCashBackAmount));
	...
	private boolean isTolerable() {
		...
		double diff = NumberUtil.roundToDigitPrice(dueAmount - (totalTenderedAmount - totalCashBackAmount));
		if (diff <= tolerance) {
			if (diff > 0) {
	...
	private void doAddToleranceToTicketDiscount(double discountAmount) {
		...
		ticketDiscount.setValue(NumberUtil.roundToDigitPrice(discountAmount));
