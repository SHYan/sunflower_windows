...
public class AutomatedWeightInputDialog extends OkCancelOptionDialog implements ActionListener {
	...
	private double convert(String unitTo, double inputValue) {
		...
		if (unitFrom.equals(UNIT_KG)) {
			if (unitTo.equals(UNIT_LB)) {
				return NumberUtil.roundToDigitQty(2.20 * inputValue);
			}
			else if (unitTo.equals(UNIT_OZ)) {
				return NumberUtil.roundToDigitQty(35.27 * inputValue);
			}
			else if (unitTo.equals(UNIT_G)) {
				return NumberUtil.roundToDigitQty(1000 * inputValue);
			}
		}
		else if (unitFrom.equals(UNIT_LB)) {
			if (unitTo.equals(UNIT_KG)) {
				return NumberUtil.roundToDigitQty(0.45 * inputValue);
			}
			else if (unitTo.equals(UNIT_OZ)) {
				return NumberUtil.roundToDigitQty(16 * inputValue);
			}
			else if (unitTo.equals(UNIT_G)) {
				return NumberUtil.roundToDigitQty(453.59 * inputValue);
			}
		}
		else if (unitFrom.equals(UNIT_G)) {
			if (unitTo.equals(UNIT_KG)) {
				return NumberUtil.roundToDigitQty(0.001 * inputValue);
			}
			else if (unitTo.equals(UNIT_OZ)) {
				return NumberUtil.roundToDigitQty(0.035 * inputValue);
			}
			else if (unitTo.equals(UNIT_LB)) {
				return NumberUtil.roundToDigitQty(0.00220462 * inputValue);
			}
		}
		else if (unitFrom.equals(UNIT_OZ)) {
			if (unitTo.equals(UNIT_KG)) {
				return NumberUtil.roundToDigitQty(0.0283495 * inputValue);
			}
			else if (unitTo.equals(UNIT_G)) {
				return NumberUtil.roundToDigitQty(28.3495 * inputValue);
			}
			else if (unitTo.equals(UNIT_LB)) {
				return NumberUtil.roundToDigitQty(0.0625 * inputValue);
			}
		}
		return inputValue;
	}
	...
	public static double takeDoubleInput(String title, double initialAmount) {
		AutomatedWeightInputDialog dialog = new AutomatedWeightInputDialog(Messages.getString("ENTER_WEIGHT"));
