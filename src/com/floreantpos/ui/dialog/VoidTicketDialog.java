	...
	private void btnVoidActionPerformed(java.awt.event.ActionEvent evt) {
		try {
			...
			if (ticket.getPaidAmount() > 0) {
				...
				double ticketTotalWithoutTips = NumberUtil.roundToDigitPrice(ticket.getTotalAmount() - tipsAmount);
				double paidAmount = ticket.getPaidAmount();

				//refundAmount = NumberSelectionDialog2.takeDoubleInput(Messages.getString("RefundAction.10"), paidAmount < ticketTotalWithoutTips ? ticket.getPaidAmount()
				//		: paidAmount - tipsAmount);
				refundAmount =  paidAmount < ticketTotalWithoutTips ? ticket.getPaidAmount() : paidAmount - tipsAmount;

				if (refundAmount == -1)
					return;

				if (tipsAmount > 0) {
					if (POSMessageDialog.showYesNoQuestionDialog(POSUtil.getFocusedWindow(), Messages.getString("WANT_REFUND"), Messages.getString("Application.2")) == JOptionPane.YES_OPTION) {
				...
				if (refundAmount > paidAmount) {
					POSMessageDialog.showMessage(POSUtil.getFocusedWindow(), Messages.getString("RefundAction.11"));
					return;
				}
			...
			/*J1.1 30*/
			ticket.setVoidedByName(Application.getCurrentUser().getFullName());
			...
			ActionHistoryDAO.getInstance().saveHistory(
					...
					"Seq:" + ticket.getId() + ";Count:"+ ticket.getItemCount()+"; Total" + ": " + NumberUtil.formatPrice(ticket.getTotalAmount())+"; Reason:"+ticket.getVoidReason()); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
