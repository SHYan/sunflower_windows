...
public class UpdateDialog extends POSDialog {
	...
	protected void initComponent() {
		...
		JLabel l = new JLabel("<html><h1>"+AppProperties.getAppName()+"</h1><h4>"+ Messages.getString("CURVERSION")+ " : " + Application.VERSION + "</h4></html>"); //$NON-NLS-1$ //$NON-NLS-2$
		panel.add(l, "cell 0 2");

		String version = "";
		if (up_to_date) {
			version = "<h2>"+Messages.getString("UPTODATE")+"</h2> ";//$NON-NLS-1$ 
		}
		else if (versions == null || versions.length == 0) {
			version = "<h4><a href='#'>"+Messages.getString("UPDATE_CHECK")+"</a></h4>";//$NON-NLS-1$
		}
		else if (versions.length > 0) {
			version = "<h2>"+Messages.getString("UPDATE_AVA")+"</h2> ";//$NON-NLS-1$ 
			for (String i : versions) {
				version += "<h4>" + i + "<a href='#'>  "+Messages.getString("DOWNLOAD")+"  </a></h4>";//$NON-NLS-1$ //$NON-NLS-2$
			}
		}
		...
		panel.add(new JLabel(Messages.getString("UPDATE_CHECK")), "split 2,cell 1 2,aligny top,right");
