...
public class LanguageSelectionDialog extends POSDialog {
	...
	private void init() {
		...
		titlePanel.setTitle(Messages.getString("LANGUAGE_SELECTION")); //$NON-NLS-1$
		...
		JLabel lblLanguage = new JLabel(Messages.getString("LANGUAGE")+" : ");

		cbLang = new JComboBox();
		cbLang.setPreferredSize(new Dimension(120, 40));
	...
	public void doSave() {
		...
		if (cbLang.getSelectedItem() != null) {
			...
			POSMessageDialog.showMessage(this, Messages.getString("LANGUAGE_DEFAULT"));
			dispose();
			try {
				Main.restart();
			} catch (Exception e) {
